...
import { Home, FilePlus, MessageCircle, Settings, LogOut } from 'lucide-react'

const NAV_ITEMS = [
  { href: '/entreprise', icon: Home, label: 'Tableau de bord' },
  { href: '/entreprise/ao', icon: FilePlus, label: 'Mes appels d\'offres' },
  { href: '/entreprise/messages', icon: MessageCircle, label: 'Messages', badge: true },
]

export default function EntrepriseLayout({ children }: { children: React.ReactNode }) {
  const [entreprise, setEntreprise] = useState<{ raison_sociale?: string } | null>(null)
    ...
    <div className="min-h-screen bg-[#fafaf8] flex">
      <aside className="w-64 bg-white border-r border-black/8 flex flex-col fixed h-full">
        <div className="p-6 border-b border-black/8">
          ...
          <p className="text-xs text-gray-500 mt-1">Entreprise</p>
          {entreprise?.raison_sociale && (
            <p className="text-xs text-gray-700 mt-1 font-medium truncate">{entreprise.raison_sociale}</p>
          )}
        </div>
        ...
        <div className="p-4 border-t border-black/8 flex flex-col gap-1">
          <Link href="/entreprise/parametres" className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-gray-600 hover:bg-gray-50 hover:text-gray-900 transition-colors">
